...
- `profiles.credits` is the current balance (starter default: **10**).
...
- Credit math happens in **server actions** with `service_role`: read balance,
  check `delta`, update `profiles.credits`, insert ledger row. Do this inside a
  transaction so balance and ledger never drift.
- Costs (from the screenshots): `runtime='hook'` = 1 credit, `runtime='full'` =
  3 credits. Keep these in the server action constants — no plan table yet.
